...
[H1(k+1),H1_(k+1),r_sim1(k+1),r_sim1_(k+1),r_meas1(k+1),rphase1(k+1),rdot_sim1(k+1),rdot_sim1_(k+1),phigt1_1(k+1),phigt2_1(k+1),phi1_1(k+1),phi2_1(k+1),phi3_1(k+1),phi4_1(k+1),offset11,offset21,offset31,offset41,offset51,offset61] = noisysimNoMultiMotion(x1,f1,Gt,X,PT,GT1,R,sigma,0.025,k,z,z_prev,phigt1_1(k),phi1_1(k),phi4_1(k),time(k)-time(k-1),magD12(k),offset11,offset21,offset31,offset41,offset51,offset61);
[H2(k+1),H2_(k+1),r_sim2(k+1),r_sim2_(k+1),r_meas2(k+1),rphase2(k+1),rdot_sim2(k+1),rdot_sim2_(k+1),phigt1_2(k+1),phigt2_2(k+1),phi1_2(k+1),phi2_2(k+1),phi3_2(k+1),phi4_2(k+1),offset12,offset22,offset32,offset42,offset52,offset62] = noisysimNoMultiMotion(x2,f2,Gt,X,PT,GT2,R,sigma,0.0025,k,z,z_prev,phigt1_2(k),phi1_2(k),phi4_2(k),time(k)-time(k-1),magD22(k),offset12,offset22,offset32,offset42,offset52,offset62);
[H3(k+1),H3_(k+1),r_sim3(k+1),r_sim3_(k+1),r_meas3(k+1),rphase3(k+1),rdot_sim3(k+1),rdot_sim3_(k+1),phigt1_3(k+1),phigt2_3(k+1),phi1_3(k+1),phi2_3(k+1),phi3_3(k+1),phi4_3(k+1),offset13,offset23,offset33,offset43,offset53,offset63] = noisysimNoMultiMotion(x3,f3,Gt,X,PT,GT3,R,sigma,0.088,k,z,z_prev,phigt1_3(k),phi1_3(k),phi4_3(k),time(k)-time(k-1),magD32(k),offset13,offset23,offset33,offset43,offset53,offset63);   
[H4(k+1),H4_(k+1),r_sim4(k+1),r_sim4_(k+1),r_meas4(k+1),rphase4(k+1),rdot_sim4(k+1),rdot_sim4_(k+1),phigt1_4(k+1),phigt2_4(k+1),phi1_4(k+1),phi2_4(k+1),phi3_4(k+1),phi4_4(k+1),offset14,offset24,offset34,offset44,offset54,offset64] = noisysimNoMultiMotion(x4,f4,Gt,X,PT,GT4,R,sigma,0.018,k,z,z_prev,phigt1_4(k),phi1_4(k),phi4_4(k),time(k)-time(k-1),magD42(k),offset14,offset24,offset34,offset44,offset54,offset64);  
...
save('SimRFPhase2.mat', 'r1', 'r2', 'r3', 'r4', 'rdot1', 'rdot2', 'rdot3', 'rdot4','rtime');


%%
% rphase2(yST:yET)=rphase2(yST:yET)+1.68;
% rphase3(yST:yET)=rphase3(yST:yET)+1.7;
% rphase4(yST:yET)=rphase4(yST:yET)+1.67;
%% ==================================================================== Results ======================================================================
...
% rmeas  = [mean(r_meas1(yST:yET) - radial(1,yST:yET)), mean(r_meas2(yST:yET) - radial(2,yST:yET)), mean(r_meas3(yST:yET) - radial(3,yST:yET)), mean(r_meas4(yST:yET) - radial(4,yST:yET))]
% rmag   = [mean(r_sim1(yST:yET)  - radial(1,yST:yET)), mean(r_sim2(yST:yET)  - radial(2,yST:yET)), mean(r_sim3(yST:yET) - radial(3,yST:yET)),  mean(r_sim4(yST:yET)  - radial(4,yST:yET))]
% rphase = [mean(rphase1(yST:yET) - radial(1,yST:yET)), mean(rphase2(yST:yET) - radial(2,yST:yET)), mean(rphase3(yST:yET) - radial(3,yST:yET)), mean(rphase4(yST:yET) - radial(4,yST:yET))]
% 
% fprintf('================================== x y z triangulation Mean Error in total ================================== ')
% meas  = sqrt(mean(coord3(1,yST:yET) - r_meas(1,yST:yET)).^2  + mean(coord3(2,yST:yET) - r_meas(2,yST:yET)).^2  + mean(coord3(3,yST:yET) - r_meas(3,yST:yET)).^2)
% mag   = sqrt(mean(coord3(1,yST:yET) - r_sim(1,yST:yET)).^2   + mean(coord3(2,yST:yET) - r_sim(2,yST:yET)).^2   + mean(coord3(3,yST:yET) - r_sim(3,yST:yET)).^2)
% phase = sqrt(mean(coord3(1,yST:yET) - r_phase(1,yST:yET)).^2 + mean(coord3(2,yST:yET) - r_phase(2,yST:yET)).^2 + mean(coord3(3,yST:yET) - r_phase(3,yST:yET)).^2)
%%
%%
...
%%
figure
subplot(4,1,1),plot(time(yST:yET), coord3(1, yST:yET),'LineWidth',2);hold on; plot(time(yST:yET), r_phase(1,yST:yET),'LineWidth',2);title('Simulated Radial Distance Derived from Phase from Reader $\#1$ in 3D','interpreter','latex');ylabel('Radial Distance [m]');xlabel('t [s]');grid on; grid minor;xlim([108, 112]);legend('Simulation','Ground truth');
subplot(4,1,2),plot(time(yST:yET), coord3(2, yST:yET),'LineWidth',2);hold on; plot(time(yST:yET), r_phase(2,yST:yET),'LineWidth',2);title('Simulated Radial Distance Derived from Phase from Reader $\#2$ in 3D','interpreter','latex');ylabel('Radial Distance [m]');xlabel('t [s]');grid on; grid minor;xlim([108, 112]);legend('Simulation','Ground truth');
subplot(4,1,3),plot(time(yST:yET), coord3(3, yST:yET),'LineWidth',2);hold on; plot(time(yST:yET), r_phase(3,yST:yET),'LineWidth',2);title('Simulated Radial Distance Derived from Phase from Reader $\#3$ in 3D','interpreter','latex');ylabel('Radial Distance [m]');xlabel('t [s]');grid on; grid minor;xlim([108, 112]);legend('Simulation','Ground truth');
