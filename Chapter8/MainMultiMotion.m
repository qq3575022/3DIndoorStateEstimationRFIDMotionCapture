...
offset51 = 0; offset52 = 0; offset53 = 0; offset54 = 0; 
offset61 = 0; offset62 = 0; offset63 = 0; offset64 = 0; 

for k = yST-1:1:yET  

[H1(k+1),H1_(k+1),r_sim1(k+1),r_sim1_(k+1),r_meas1(k+1),rphase1(k+1),rdot_sim1(k+1),rdot_sim1_(k+1),phigt1_1(k+1),phigt2_1(k+1),phi1_1(k+1),phi2_1(k+1),phi3_1(k+1),phi4_1(k+1),offset11,offset21,offset31,offset41,offset51,offset61] = noisysimMultiMotion(x1,f1,Gt,X,PT,GT1,R,sigma,0.025, k,z,z_prev,phigt1_1(k),phi1_1(k),phi4_1(k),time(k+1)-time(k),magD12(k),offset11,offset21,offset31,offset41,offset51,offset61);
[H2(k+1),H2_(k+1),r_sim2(k+1),r_sim2_(k+1),r_meas2(k+1),rphase2(k+1),rdot_sim2(k+1),rdot_sim2_(k+1),phigt1_2(k+1),phigt2_2(k+1),phi1_2(k+1),phi2_2(k+1),phi3_1(k+1),phi4_2(k+1),offset12,offset22,offset32,offset42,offset52,offset62] = noisysimMultiMotion(x2,f2,Gt,X,PT,GT2,R,sigma,0.0025,k,z,z_prev,phigt1_2(k),phi1_2(k),phi4_2(k),time(k+1)-time(k),magD22(k),offset12,offset22,offset32,offset42,offset52,offset62);
[H3(k+1),H3_(k+1),r_sim3(k+1),r_sim3_(k+1),r_meas3(k+1),rphase3(k+1),rdot_sim3(k+1),rdot_sim3_(k+1),phigt1_3(k+1),phigt2_3(k+1),phi1_3(k+1),phi2_3(k+1),phi3_1(k+1),phi4_3(k+1),offset13,offset23,offset33,offset43,offset53,offset63] = noisysimMultiMotion(x3,f3,Gt,X,PT,GT3,R,sigma,0.088, k,z,z_prev,phigt1_3(k),phi1_3(k),phi4_3(k),time(k+1)-time(k),magD32(k),offset13,offset23,offset33,offset43,offset53,offset63);   
[H4(k+1),H4_(k+1),r_sim4(k+1),r_sim4_(k+1),r_meas4(k+1),rphase4(k+1),rdot_sim4(k+1),rdot_sim4_(k+1),phigt1_4(k+1),phigt2_4(k+1),phi1_4(k+1),phi2_4(k+1),phi3_1(k+1),phi4_4(k+1),offset14,offset24,offset34,offset44,offset54,offset64] = noisysimMultiMotion(x4,f4,Gt,X,PT,GT4,R,sigma,0.018, k,z,z_prev,phigt1_4(k),phi1_4(k),phi4_4(k),time(k+1)-time(k),magD42(k),offset14,offset24,offset34,offset44,offset54,offset64);  


rphase1(k+1) = rphase1(k+1)+0.4;%+30.3184;%+0.9184-1.0+30.4;
rphase2(k+1) = rphase2(k+1)+0.45;%+30.4;%+3.8;
rphase3(k+1) = rphase3(k+1)+0.3;%-0.4;
rphase4(k+1) = rphase4(k+1)+0.3;%+3.8;
...
save('SimRFPhase4.mat', 'r1', 'r2', 'r3', 'r4', 'rdot1', 'rdot2', 'rdot3', 'rdot4','rtime');
%% ==================================================================== Results ======================================================================
...
% rmeas  = 1000*[mean(r_meas1(yST:yET) - radial(1,yST:yET)), mean(r_meas2(yST:yET) - radial(2,yST:yET)), mean(r_meas3(yST:yET) - radial(3,yST:yET)), mean(r_meas4(yST:yET) - radial(4,yST:yET))]
% rmag   = 1000*[mean(r_sim1(yST:yET)  - radial(1,yST:yET)), mean(r_sim2(yST:yET)  - radial(2,yST:yET)), mean(r_sim3(yST:yET) - radial(3,yST:yET)),  mean(r_sim4(yST:yET)  - radial(4,yST:yET))]
% rphase = 1000*[mean(rphase1(yST:yET) - radial(1,yST:yET)), mean(rphase2(yST:yET) - radial(2,yST:yET)), mean(rphase3(yST:yET) - radial(3,yST:yET)), mean(rphase4(yST:yET) - radial(4,yST:yET))]
% 
% fprintf('================================== x y z triangulation Mean Error in total ================================== ')
% meas  = 1000*sqrt(mean(coord3(1,yST:yET) - r_meas(1,yST:yET)).^2  + mean(coord3(2,yST:yET) - r_meas(2,yST:yET)).^2  + mean(coord3(3,yST:yET) - r_meas(3,yST:yET)).^2)
% mag   = 1000*sqrt(mean(coord3(1,yST:yET) - r_sim(1,yST:yET)).^2   + mean(coord3(2,yST:yET) - r_sim(2,yST:yET)).^2   + mean(coord3(3,yST:yET) - r_sim(3,yST:yET)).^2)
% phase = 1000*sqrt(mean(coord3(1,yST:yET) - r_phase(1,yST:yET)).^2 + mean(coord3(2,yST:yET) - r_phase(2,yST:yET)).^2 + mean(coord3(3,yST:yET) - r_phase(3,yST:yET)).^2)
...
%%
figure
subplot(4,1,1),plot(time(yST:yET), r_phase(1,yST:yET),'LineWidth',2);hold on; plot(time(yST:yET), coord3(1,yST:yET),'LineWidth',2);title('Simulated Radial Distance Derived from Phase from Reader $\#1$ in 3D','interpreter','latex');ylabel('Radial Distance [m]');xlabel('t [s]');grid on; grid minor;xlim([108, 112]);legend('Simulation','Ground truth');
...
subplot(4,1,3),plot(time(yST:yET), r_phase(3,yST:yET),'LineWidth',2);hold on; plot(time(yST:yET), coord3(3,yST:yET),'LineWidth',2);title('Simulated Radial Distance Derived from Phase from Reader $\#3$ in 3D','interpreter','latex');ylabel('Radial Distance [m]');xlabel('t [s]');grid on; grid minor;xlim([108, 112]);legend('Simulation','Ground truth');
